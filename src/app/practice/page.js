...
import useAuth from '@/hooks/useAuth'; // استيراد الحارس
...
export default function PracticePage() {
  const { user, loading: authLoading } = useAuth(); // استخدام الحارس للحماية وجلب المستخدم
  ...
  useEffect(() => {
    // جلب الوحدات الدراسية فقط، لأن الحارس يجلب المستخدم
          ...
          <div className="max-w-4xl mx-auto bg-white p-8 rounded-xl shadow-lg">
              ...
              {currentQuestion.options.map((option, index) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full p-4 text-left rounded-lg border-2 transition-colors duration-200 flex items-center justify-between disabled:cursor-not-allowed ${feedbackClass} ${!showFeedback ? 'hover:bg-gray-100' : ''}`}
                  >
                    <span>{option}</span>
            ...
            {showFeedback && (
              <div className="mt-6 bg-gray-50 p-4 rounded-lg border">
                ...
                <button onClick={() => handleNextQuestion(true)} className="mt-4 px-6 py-2 bg-blue-600 text-white font-semibold rounded-md hover:bg-blue-700">
