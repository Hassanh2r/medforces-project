...
export default function ContestPage({ params }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const { data: challengeData } = await supabase
        .from('challenges').select('*, contest_questions(questions(*))').eq('id', resolvedParams.id).single();
      ...
      if (challengeData) {
        ...
        const contestQuestions = (challengeData.contest_questions || []).map(cq => cq.questions).filter(Boolean);
  ...
  const handleNextQuestion = async () => {
    ...
    if (nextIndex < questions.length) {
      ...
      if (user) {
        await supabase.from('quiz_results').insert([{ user_id: user.id, challenge_id: challenge.id, score: finalScore, total_time_seconds: totalTimeSeconds }]);
      }
    ...
    return (
      <div>
        ...
        <div className="mt-8 flex justify-end">
          <button onClick={handleNextQuestion} disabled={selectedAnswer === null} className="px-8 py-2 bg-green-600 text-white font-semibold rounded-lg shadow-md hover:bg-green-700 disabled:bg-gray-400">
  ...
  const renderContent = () => {
    ...
    switch (contestState) {
      ...
      default:
        return <p className="text-center animate-pulse">Calculating contest status...</p>;
    }
  };
